...
class _Store:
    ...
    # Explore clears the archive flag before running; this KB is live, so the
    # helper reads these and flips nothing.
    def list_projects(self, *, include_archived=False):
        return [
            {
                "project_id": _Ctx.project_id,
                "archived_at": None,
                "kbs": [{"kb_id": _Ctx.kb_id, "archived_at": None}],
            }
        ]

    def set_archived(self, **patch):
        self.archive_calls.append(patch)
